<script lang="ts">
    import { Keyboard, Zap, Settings, Terminal, Plus, RefreshCw } from '@lucide/svelte';
    ...
    <header class="bg-white border-b border-gray-200 px-8 py-4 flex-shrink-0">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <button
                    class="px-3 py-1.5 text-sm text-gray-600 bg-gray-100 hover:bg-gray-200 rounded transition-colors cursor-pointer"
                >
                    测试热键
                </button>
                <button
                ...
                <div class="space-y-1">
                    {#each menuItems as item (item.id)}
                        ...
                        <button
                            ...
                            class="w-full flex flex-col items-center gap-1.5 px-2 py-3 rounded-lg transition-colors cursor-pointer {currentView ===
                            item.id
                                ? 'bg-blue-50 text-blue-600'
                                : 'text-gray-600 hover:bg-gray-50'}"
                        >
                            <div
                                class="w-8 h-8 {item.color} rounded-lg flex items-center justify-center"
                            >
