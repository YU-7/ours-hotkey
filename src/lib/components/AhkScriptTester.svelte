...
<div class="bg-white border border-gray-200 rounded-lg p-4">
  ...
  <div class="space-y-3">
    ...
    <!-- 状态显示 -->
    {#if ahkStatus}
      <div class="p-3 rounded-lg text-xs {ahkStatus.includes('失败') || ahkStatus.includes('❌') ? 'bg-red-50 border border-red-200 text-red-700' : 'bg-green-50 border border-green-200 text-green-700'}">
    ...
    {#if runningScripts}
      <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <pre class="whitespace-pre-wrap text-xs text-blue-700 font-mono">{runningScripts}</pre>
      </div>
    {/if}

    {#if pathTestResult}
      <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <pre class="whitespace-pre-wrap text-xs text-blue-700 font-mono">{pathTestResult}</pre>
      </div>
    {/if}
  </div>
</div>
